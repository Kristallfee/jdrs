...
int TMrfGal_Udp::doIoctl(const int &requestType, void *arg) const
{
    ...
    toSend.push_back(req->dataFlag);
    toSend.push_back(req->registerAddress);
    toSend.push_back(req->wordcount/5);
...
int TMrfGal_Udp::_readBuffer(char* const dataStartAddress, u_int32_t length, u_int64_t& cmdword, u_int32_t* wordcount) const
{
    ...
    if ( length > MAX_BLOCKS_PER_UDP_PACKAGE*5 )
        length = MAX_BLOCKS_PER_UDP_PACKAGE*5;
    ...
    if ( UDPpollInfoResult == -1 ) {
        ...
        const size_t recvLength = MAX_BLOCKS_PER_UDP_PACKAGE*5;

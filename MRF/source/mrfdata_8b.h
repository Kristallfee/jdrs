...
class TMrfData_8b
{
	public:
		...
		*/
		void setWord(const u_int32_t& position, const u_int8_t& dataword, const bool& truncate_ok = false);
		...
		<b>Error codes:</b>
			...
			- \b mrfdata_8b_error::out_of_bounds: The data word had to be truncated to fit into the data stream.
		*/
		void setWordMasked(const u_int32_t& position, const u_int8_t& dataword, const u_int8_t& mask = ~0, const bool& truncate_ok = false);
		...
		*/
		const u_int8_t& getWord(const u_int32_t& position) const;
		
		const u_int8_t& appendWord(const u_int8_t &dataword);
...
//	protected:
		...
		*/
		...
		static const u_int32_t bitsinablock = sizeof(u_int8_t) * CHAR_BIT;

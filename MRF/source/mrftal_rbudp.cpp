...
int TMrfTal_RBUDP::readOutputBuffer(TMrfData_8b& data, const u_int32_t& wordcount, const u_int32_t& appendat, const bool& truncate) const
{
    if (deviceIsOpen()) {
        int words_read;
...
int TMrfTal_RBUDP::doReadout(const u_int32_t& wordcount, u_int8_t* const startaddress) const
{
    if (deviceIsOpen()) {
        ...
        if( TMrfGal_Udp::doIoctl(req.dataFlag, &req) <0)
        {
        return -1 ;
        }
...
void TMrfTal_RBUDP::readRemoteData(TMrfData_8b& data) const
{
